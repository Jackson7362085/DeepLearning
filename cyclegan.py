...
class CycleGAN():
    def __init__(self):
       ...
       # config.gpu_options.per_process_gpu_memory_fraction = 0.95
        ...
        self.combined = Model(inputs=[img_A, img_B],
                              outputs=[ valid_A, valid_B,   # 生成器生成的图片应让判别器无法区分
                                        reconstr_A, reconstr_B,
                                        img_A_id, img_B_id ])
        self.combined.compile(loss=['mse', 'mse',           # 均方误差
                                    'mae', 'mae',           # 平均绝对误差
                                    'mae', 'mae'],
                            loss_weights=[0.5, 0.5,
                                        self.lambda_cycle, self.lambda_cycle, # 5
                                        self.lambda_id, self.lambda_id ],     # 2.5
                            optimizer=optimizer)
    ...
    def sample_images(self, epoch, batch_i):
        ...
        gen_imgs = np.concatenate([imgs_A, fake_B, imgs_B, fake_A])  # 拼接

        gen_imgs = 0.5 * gen_imgs + 0.5                              # 恢复原始图像
...
gan = CycleGAN()
gan.train(init_epoch=10, epochs=200, batch_size=1, sample_interval=200)
